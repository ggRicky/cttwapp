    ...
    <div class="row">
        <div class="col-lg-12 text-justify yii2-content">
            ...
            <?php if (\Yii::$app->user->can('listBrand')): ?>
                ...
                <!-- 2018-06-15 : Begin the ajax functionality to refresh only the GridView widget contents. -->

                <?php Pjax::begin(); ?>

                    <p>
                        <?= Html::a(Yii::t('app', 'Crear Marca'), ['create', 'page'=>$curr_page], ['class' => 'btn btn-success', 'title' => Yii::t('app', 'Crear un nuevo registro de marca')]) ?>
                    </p>
                            ...
                            'columns' => [
                                [
                                    ...
                                    'urlCreator' => function ($action, $model)  use ($dataProvider) {
                                        if ($action === 'delete') {
                                            $url = Url::to(['brand/delete', 'id' => $model->id, 'page' => ($dataProvider->pagination->page + 1)]);
                                        }
                                        elseif ($action === 'view') {
                                            $url = Url::to(['brand/view', 'id' => $model->id, 'page' => ($dataProvider->pagination->page + 1)]);
                                        }
                                        elseif ($action === 'update') {
                                            $url = Url::to(['brand/update', 'id' => $model->id, 'page' => ($dataProvider->pagination->page + 1)]);
                                        }
                                        else $url = null;
